...
// El registro reusa el panel de personas (backend probado: alta -> fichadores +
// Odoo). Se embebe tal cual, pero SIN su pantalla de "Clave de acceso": el server
// entra por nosotros y le pasa un token ya válido al iframe. Lo habilita la
// sección "registro" del usuario.
async function abrirRegistro() {
  const fr = $("#registro-frame");
  if (!fr || fr.dataset.cargado) return;      // se carga una sola vez por sesión
  try {
    const { token, scheme, port } = await apiSede("/registro/panel");
    fr.src = `${scheme}://${location.hostname}:${port}/?panelToken=${encodeURIComponent(token)}`;
  } catch (e) {
    // Sin token (panel apagado o mal configurado): se abre igual, con su login.
    fr.src = `http://${location.hostname}/`;
  }
  fr.dataset.cargado = "1";
}
